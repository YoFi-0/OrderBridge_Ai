...
declare module "express" {
    interface Request {
        session: Session & { [key: string]: any }; // This is more flexible/less strict
    }
}
...
const isLogin = (req: Request, res: Response, next: NextFunction) => {
    if (req.session && req.session.user && (req.session.user as any).isAdmin) {
        next();
        return;
    }
    res.redirect("/login");
...
app.post("/clients/update/:id", isLogin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, phoneNumber, apiKey, GeminiKey, WhatsAppKey, isBan } =
            req.body;
        ...
        if (isBan === "true") {
            ioDisconnectSocket(
                tempSocket.find((x) => x.phoneNumber == phoneNumber)
                    ?.socketId || "",
            );
        }
        res.redirect("/clients");
...
export const ioSearch = (socketId: string, userMsg: any) => {
    console.log("search socket is running !!");
    io.to(socketId).emit("search", userMsg);
};
export const ioDisconnectSocket = (socketId?: string) => {
    if (!socketId) {
        return;
    }
...
export const ioOnProductFound = async (socketId: string) => {
    ...
    const product = await new Promise((resolve) => {
        ...
        io.sockets.sockets
            .get(socketId)
            ?.once("product_found", (data: Product) => {
...
io.on("connection", async (socket) => {
    ...
    socket.on("temp_save", async (data) => {
        ...
        if (isFound.get().isBan) {
            socket.emit(
                "server_error",
                "you have no access to our service contact with support !!",
            );
        }
        tempSocket.push({
            socketId: socket.id,
